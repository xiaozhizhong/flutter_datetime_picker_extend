...
///@author shaw
///@date 2021/2/25
///@desc 修改自 https://github.com/huuang/lunar_calendar_converter/blob/master/lib/lunar_solar_converter.dart
///
class LunarSolarConverter {
  ...
  static CalendarDate _solarFromInt(int data) {
    ...
    return CalendarDate.solar(year: y, month: mm, day: dd);
  }

  static CalendarDate lunarToSolar(CalendarDate lunar) {
    int days = _lunar_month_days[lunar.year - _lunar_month_days[0]];
    int leap = _getBitInt(days, 4, 13);
    int offset = 0;
    int loop_end = leap;

    if (!lunar.isLunarLeap) {
  ...
  static CalendarDate solarToLunar(CalendarDate solar) {
    int index = solar.year - _solar_1_1[0];
    ...
    int m = _getBitInt(solar11, 4, 5);
    int d = _getBitInt(solar11, 5, 0);
    int offset = _solarToInt(solar.year, solar.month, solar.day) - _solarToInt(y, m, d);
    ...
    var day = lunarD;
    return CalendarDate.lunar(year: year, month: month, day: day, isLunarLeap: isLeap)
      ..hour = solar.hour
      ..minute = solar.minute
      ..second = solar.second;
  }
