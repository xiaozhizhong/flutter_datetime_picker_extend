...
///@author shaw
///@date 2021/2/24
///@desc 农历辅助 copy from https://github.com/wskfjtheqian/chinese_calendar/blob/master/lib/src/calendar_utils.dart
///
class DateHelper {
  DateHelper._();
  ...
  /// @Array Of Property
  /// @return Number
  static const List<int> _solarMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

  /// 返回农历y年一整年的总天数
  static int lunarYearDays(int year) {
    var i, sum = 348;
  ...
  static int lunarLeapMonth(int year) {
    return (_lunarInfo[year - 1900] & 0xf);
  }

  /// 返回农历y年闰月的天数 若该年没有闰月则返回0
  static int lunarLeapMonthDays(int year) {
    if (0 != lunarLeapMonth(year)) {
  ...
  static int lunarNonLeapMonthDays(int year, int month) {
    if (month > 12 || month < 1) {
      return -1;
    }
  ...
  /// 返回农历y年m月的总天数
  static int lunarMonthDays(int year, int month, bool isLeap) {
    if (isLeap) return lunarLeapMonthDays(year);
    return lunarNonLeapMonthDays(year, month);
  }

  /// 返回公历(!)y年m月的天数
  static int solarMonthDays(int year, int month) {
    if (month > 12 || month < 1) {
      return -1;
    }
    var ms = month - 1;
...
//
// /// 阳历节日
// static const Map<String, String> _festival = {
//   '1-1': '元旦节',
//   '2-14': '情人节',
//   '5-1': '劳动节',
//   '5-4': '青年节',
...
//
// /// 农历节日
// static const Map<String, String> _lfestival = {
//   '12-30': '除夕',
//   '1-1': '春节',
//   '1-15': '元宵节',
//   '5-5': '端午节',
...
// ];
//
// static const List<String> _nStr1 = ['日', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
//
// static const  List<String> _nStr2 = ['初', '十', '廿', '卅'];
// static const List<String> _nStr3 = ['正', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊'];
// static const List<String> _nStr4 = ['魔羯', '水瓶', '双鱼', '白羊', '金牛', '双子', '巨蟹', '狮子', '处女', '天秤', '天蝎', '射手', '魔羯'];
...
// static DateTime lunar2solar(DateTime dateTime, bool isLeapMonth) {
//   int y = dateTime.year;
